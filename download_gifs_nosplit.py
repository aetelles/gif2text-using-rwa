...
def dl_process_data(save_path, row, debug):
    ...
    if os.path.exists(full_path):
        ...
        try:
            while 1:
                gif.seek(gif.tell() + 1)
                frame_filename = 'frame_' + str(frame_count) + '.jpgs'
...
#   in case of bugs, check this line, and all the commented lines above
#   it may also help to check the download_gifs.py in ubuntu because that may be a different version
    tgif_data.apply(lambda row: dl_process_data(train_path, row, debug), axis=1)
